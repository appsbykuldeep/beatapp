import 'package:beatapp/ui/dialog/dialog_helper.dart';
import 'package:beatapp/utility/camera_and_file_provider.dart';
import 'package:beatapp/utility/date_time_helper.dart';
import 'package:beatapp/utility/json_to_excel.dart';
import 'package:beatapp/utility/message_utility.dart';
import 'package:flutter/cupertino.dart';
import 'package:intl/intl.dart';
import 'package:syncfusion_flutter_xlsio/xlsio.dart';

class PendingEmployeeListConsResponse {
  //@SerializedName("EMPLOYEE_SR_NUM")
  String beatName;
  String? eMPLOYEESRNUM;

  //@SerializedName("REQUESTING_AGENCY_NAME")
  String? rEQUESTINGAGENCYNAME;

  //@SerializedName("REG_DT")
  String? rEGDT;

  //@SerializedName("ASSIGNED_ON")
  String? aSSIGNEDON;

  //@SerializedName("TARGET_DT")
  String? tARGETDT;

  //@SerializedName("EMPLOYEE_NAME")
  String? eMPLOYEENAME;

  //@SerializedName("EO_NAME")
  String? eONAME;

  PendingEmployeeListConsResponse(
      this.beatName,
      this.eMPLOYEESRNUM,
      this.rEQUESTINGAGENCYNAME,
      this.rEGDT,
      this.aSSIGNEDON,
      this.tARGETDT,
      this.eMPLOYEENAME,
      this.eONAME);

  factory PendingEmployeeListConsResponse.fromJson(json) {
    return PendingEmployeeListConsResponse(
        json["BEAT_NAME"]??'',
        json["EMPLOYEE_SR_NUM"].toString(),
        json["REQUESTING_AGENCY_NAME"].toString(),
        json["REG_DT"].toString(),
        json["ASSIGNED_ON"].toString(),
        json["TARGET_DT"].toString(),
        json["EMPLOYEE_NAME"].toString(),
        json["EO_NAME"].toString());
  }

  static List<PendingEmployeeListConsResponse> getLast15DaysList(List<PendingEmployeeListConsResponse> inputlist) {
    List<PendingEmployeeListConsResponse> outputList =
    inputlist.where((PendingEmployeeListConsResponse element) {
      try {
        final date =
        DateFormat('dd/MM/yyyy').parse(element.aSSIGNEDON.toString());
        return DateTimeHelper.get15days().isBefore(date);
      } catch (e) {
        return false;
      }
    }).toList();
    return outputList;
  }

  static List<PendingEmployeeListConsResponse> getLast15To30DaysList(List<PendingEmployeeListConsResponse> inputlist) {
    List<PendingEmployeeListConsResponse> outputList =
    inputlist.where((PendingEmployeeListConsResponse element) {
      try {
        final date =
        DateFormat('dd/MM/yyyy').parse(element.aSSIGNEDON.toString());
        return DateTimeHelper.get15days().isAfter(date) &&
            DateTimeHelper.getBefore30day().isBefore(date);
      } catch (e) {
        return false;
      }
    }).toList();
    return outputList;
  }

  static List<PendingEmployeeListConsResponse> getBefore30DaysList(
      List<PendingEmployeeListConsResponse> inputlist) {
    List<PendingEmployeeListConsResponse> outputList =
    inputlist.where((PendingEmployeeListConsResponse element) {
      try {
        final date =
        DateFormat('dd/MM/yyyy').parse(element.aSSIGNEDON.toString());
        return DateTimeHelper.getBefore30day().isAfter(date);
      } catch (e) {
        return false;
      }
    }).toList();
    return outputList;
  }

  static List<PendingEmployeeListConsResponse> getDataFromToDateList(String fDate,String tDate,List<PendingEmployeeListConsResponse> inputlist) {
    List<PendingEmployeeListConsResponse> outputList = inputlist.where((PendingEmployeeListConsResponse element) {
      try {
        final date = DateFormat('dd/MM/yyyy').parse(element.aSSIGNEDON.toString());
        return DateFormat('MM/dd/yyyy')
            .parse(fDate).isBefore(date) &&
            DateFormat('MM/dd/yyyy')
                .parse(tDate).isAfter(date);
      } catch (e) {
        return false;
      }
    }).toList();
    return outputList;
  }

  static Future<void> generateExcel(
      context, List<PendingEmployeeListConsResponse> lst) async {
    if (lst.isNotEmpty) {
      DialogHelper.showLoaderDialog(context);
      //Creating a workbook.
      final Workbook workbook = Workbook();
      //Accessing via index
      final Worksheet sheet = workbook.worksheets[0];
      sheet.showGridlines = true;

      // Enable calculation for worksheet.
      sheet.enableSheetCalculations();
      JsonToExcel.setExcelStyle(sheet);

      sheet.getRangeByIndex(1, 1).setText('EMPLOYEE_SR_NUM');
      sheet.getRangeByIndex(1, 2).setText('REGISTERED_DT');
      sheet.getRangeByIndex(1, 3).setText('ASSIGNED_DT');
      sheet.getRangeByIndex(1, 4).setText('TARGET_DT');
      sheet.getRangeByIndex(1, 5).setText('BEAT_CONSTABLE_NAME');
      sheet.getRangeByIndex(1, 6).setText('EMPLOYEE_NAME');
      sheet.getRangeByIndex(1, 7).setText('REQUESTING_AGENCY_NAME');
      sheet.getRangeByIndex(1, 8).setText('EO_NAME');

      for (int i = 0; i < lst.length; i++) {
        sheet
            .getRangeByIndex(i + 2, 1)
            .setText((lst[i].eMPLOYEESRNUM ?? "").toString());
        sheet
            .getRangeByIndex(i + 2, 2)
            .setText((lst[i].rEGDT ?? "").toString());
        sheet
            .getRangeByIndex(i + 2, 3)
            .setText((lst[i].aSSIGNEDON ?? "").toString());
        sheet
            .getRangeByIndex(i + 2, 4)
            .setText((lst[i].tARGETDT ?? "").toString());
        sheet
            .getRangeByIndex(i + 2, 5)
            .setText(("").toString());
        sheet
            .getRangeByIndex(i + 2, 6)
            .setText((lst[i].eMPLOYEENAME ?? "").toString());
        sheet
            .getRangeByIndex(i + 2, 7)
            .setText((lst[i].rEQUESTINGAGENCYNAME ?? "").toString());
        sheet
            .getRangeByIndex(i + 2, 8)
            .setText((lst[i].eONAME ?? "").toString());
      }

      //Save and launch the excel.
      final List<int> bytes = workbook.saveAsStream();
      //Dispose the document.
      workbook.dispose();

      Navigator.pop(context);

      //Save and launch the file.
      await CameraAndFileProvider.saveBytesInStorage(
          'PendingEmpVer', bytes, ".xlsx");
      MessageUtility.showDownloadCompleteMsg(context);
    } else {
      MessageUtility.showNoDataMsg(context);
    }
  }
}
