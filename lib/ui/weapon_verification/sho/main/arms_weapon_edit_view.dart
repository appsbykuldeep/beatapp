...
class ArmsWeaponEditActivity extends StatefulWidget {
  final data;

  const ArmsWeaponEditActivity({Key? key, required this.data})
      : super(key: key);

  @override
  State<ArmsWeaponEditActivity> createState() =>
      _ArmsWeaponEditActivityState(data);
}

class _ArmsWeaponEditActivityState extends State<ArmsWeaponEditActivity> {
  ...
  void _getWeaponDeatils() async {
    ...
    if (response.statusCode == 200) {
      WeaponDetail_Table armDetails =
          WeaponDetail_Table.fromJson(response.data["Table"][0]);
      con_LicHol_Name.text = armDetails.lISCENSEHOLDERNAME ?? "";
      con_fName.text = armDetails.fATHERNAME ?? "";
      con_Age.text = armDetails.aGE ?? "";
      con_Mobile.text = armDetails.mOBILE ?? "";
      con_Add.text = armDetails.aDDRESS ?? "";
      con_WeaponModel.text = armDetails.aRMSMODEL ?? "";
      con_Weapon_Lic_No.text = armDetails.aRMSLISCENSENO ?? "";
      dropdownValueBeat =
          "${armDetails.bEATNAME.trim()}#${armDetails.BEAT_CD!.trim()}";
      dropdownValueVillStr =
          "${armDetails.vILLSTREETNAME!.trim()}#${armDetails.VILL_STR_SR_NUM!.trim()}";
      dropdownValueWeaponType =
          "${armDetails.wEAPON!.trim()}#${armDetails.WEAPON_SUBTYPE_CD!.trim()}";
      selectedDate = armDetails.wEAPONEXPIRTDATE ?? "";
      _getBeatList();
      _getVillageList();
      _getWeaponTypeList();
    } else {
    ...
    if (response.statusCode == 200) {
      try {
        for (Map<String, dynamic> i in response.data) {
          ...
          spinnerItemsdropdownValueWeaponType.add(
              "${data.WEAPON_SUBTYPE!.trim()}#${data.WEAPON_SUBTYPE_CD!.trim()}");
        }
        setState(() {});
      } catch (e) {
        e.toString();
  ...
  void _saveData() async {
    ...
    var response = await HttpRequst.postRequestWithTokenAndBody(
        context, EndPoints.SUBMIT_ARMS_WEAPON_UPDATE, data, false);
