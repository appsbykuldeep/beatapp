...
import 'package:beatapp/model/response/pending_tenant_response.dart';
...
class PendingFragmentTenant extends StatefulWidget {
  const PendingFragmentTenant({Key? key}) : super(key: key);

  @override
  State<PendingFragmentTenant> createState() => _PendingFragmentTenantState();
}

class _PendingFragmentTenantState extends State<PendingFragmentTenant> {
  List<PendingTenantResponse> _lstData = [];
  final List<PendingTenantResponse> _lstDataAll = [];
  ...
  void _getTenantVerificationList() async {
    ...
    var response = await HttpRequst.postRequestWithTokenAndBody(
        context, EndPoints.END_POINT_ASSIGNED_TENENT_LIST, data, true);
    if (response.statusCode == 200) {
      try {
        for (Map<String, dynamic> i in response.data) {
          var data = PendingTenantResponse.fromJson(i);
          _lstData.add(data);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: InkWell(
        onTap: () {
          PendingTenantResponse.generateExcel(context, _lstData);
        },
        child: Container(
          height: 50,
          color: Colors.white,
      ...
      child: Container(
          ...
          child: Row(
            children: [
              ...
              SizedBox(
                width: 130,
                child: Text((data.aSSIGNEDDT ?? "").split(" ")[0], maxLines: 2),
              ),
              SizedBox(
                width: 130,
                    ...
                    Container(
                      ...
                      child: InkWell(
                        onTap: () {
                          ...
                          _lstData =
                              PendingTenantResponse.getDataFromToDateList(
  ...
  setFilterData() {
    ...
    if (checkedFilter == 1) {
      ...
      _lstData = PendingTenantResponse.getLast15DaysList(_lstDataAll);
    } else if (checkedFilter == 3) {
      _lstData = PendingTenantResponse.getLast15To30DaysList(_lstDataAll);
    } else if (checkedFilter == 4) {
      _lstData = PendingTenantResponse.getBefore30DaysList(_lstDataAll);
    } else {
      _lstData = _lstDataAll;
