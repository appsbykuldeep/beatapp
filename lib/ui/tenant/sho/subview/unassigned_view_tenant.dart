...
import 'package:beatapp/model/response/unassigned_tenant_response.dart';
import 'package:beatapp/ui/dialog/dialog_helper.dart';
import 'package:beatapp/ui/tenant/sho/tenant_verification_assign_view.dart';
import 'package:beatapp/utility/message_utility.dart';
import 'package:beatapp/utility/resource_provider.dart';
import 'package:flutter/material.dart';
import 'package:beatapp/api/api_connection.dart' as HttpRequst;

class UnassignedFragmentTenant extends StatefulWidget {
  const UnassignedFragmentTenant({Key? key}) : super(key: key);

  @override
  State<UnassignedFragmentTenant> createState() =>
      _UnassignedFragmentTenantState();
}

class _UnassignedFragmentTenantState extends State<UnassignedFragmentTenant> {
  List<UnassignedTenantResponse> _lstData = [];
  final List<UnassignedTenantResponse> _lstDataAll = [];
  ...
  void _getTenantVerificationList() async {
    ...
    var response = await HttpRequst.postRequestWithTokenAndBody(
        context, EndPoints.END_POINT_UNASSIGNED_TENENT_LIST, data, true);
    if (response.statusCode == 200) {
      try {
        for (Map<String, dynamic> i in response.data) {
          var data = UnassignedTenantResponse.fromJson(i);
          _lstData.add(data);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _lstData.isNotEmpty
                              ...
                              child: Row(
                                children: [
                                  ...
                                  SizedBox(
                                    width: 130,
                                    child: Text(
                                      getTranlateString("name_of_landlord"),
                                      style: const TextStyle(
                                          fontWeight: FontWeight.bold,
                                          fontSize: 12),
                                    ),
          ...
          : Center(
              child:
                  Text(AppTranslations.of(context)!.text("no_record_found"))),
      bottomNavigationBar: InkWell(
        onTap: () {
          UnassignedTenantResponse.generateExcel(context, _lstData);
        },
        child: Container(
          height: 50,
          color: Colors.white,
    ...
    return InkWell(
      onTap: () async {
        await Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => TenantVerificationAssignActivity(
      ...
      child: Container(
          ...
          child: Row(
            children: [
              ...
              SizedBox(
                width: 130,
                child: Text(data.tENANTSRNUM ?? "", maxLines: 2),
              ),
              SizedBox(
                width: 130,
                child: Text((data.rEGISTRATIONDATE ?? "").split(" ")[0],
                    maxLines: 2),
              ),
              SizedBox(
                width: 130,
                    ...
                    Container(
                      ...
                      child: InkWell(
                        onTap: () {
                          ...
                          _lstData =
                              UnassignedTenantResponse.getDataFromToDateList(
  ...
  setFilterData() {
    ...
    if (checkedFilter == 1) {
      ...
      _lstData = UnassignedTenantResponse.getLast15DaysList(_lstDataAll);
    } else if (checkedFilter == 3) {
      _lstData = UnassignedTenantResponse.getLast15To30DaysList(_lstDataAll);
    } else if (checkedFilter == 4) {
      _lstData = UnassignedTenantResponse.getBefore30DaysList(_lstDataAll);
    } else {
      _lstData = _lstDataAll;
