...
import 'package:beatapp/model/response/completed_tenant_response.dart';
...
class CompletedFragmentTenant extends StatefulWidget {
  const CompletedFragmentTenant({Key? key}) : super(key: key);

  @override
  State<CompletedFragmentTenant> createState() =>
      _CompletedFragmentTenantState();
}

class _CompletedFragmentTenantState extends State<CompletedFragmentTenant> {
  List<CompletedTenantResponse> _lstData = [];
  final List<CompletedTenantResponse> _lstDataAll = [];
  ...
  void _getTenantVerificationList() async {
    ...
    var response = await HttpRequst.postRequestWithTokenAndBody(
        context, EndPoints.END_POINT_COMPLETED_TENENT_LIST, data, true);
    if (response.statusCode == 200) {
      try {
        for (Map<String, dynamic> i in response.data) {
          var data = CompletedTenantResponse.fromJson(i);
          _lstData.add(data);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: InkWell(
        onTap: () {
          CompletedTenantResponse.generateExcel(context, _lstData);
        },
        child: Container(
          height: 50,
          color: Colors.white,
        ...
        Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) => TenantVerificationDetailActivity(
                  data: {"TENANT_SR_NUM": data.tENANTSRNUM, "action": "1"}),
            ));
      },
      child: Container(
          padding: const EdgeInsets.only(left: 4, right: 4),
          ...
          child: Row(
            children: [
              ...
              SizedBox(
                width: 130,
                child:
                    Text((data.rEGISTEREDDT ?? "").split(" ")[0], maxLines: 2),
              ),
              SizedBox(
                width: 130,
                child:
                    Text((data.cOMPLETEDDT ?? "").split(" ")[0], maxLines: 2),
              ),
              SizedBox(
                width: 130,
                    ...
                    Container(
                      ...
                      child: InkWell(
                        onTap: () {
                          ...
                          _lstData =
                              CompletedTenantResponse.getDataFromToDateList(
  ...
  setFilterData() {
    ...
    if (checkedFilter == 1) {
      ...
      _lstData = CompletedTenantResponse.getLast15DaysList(_lstDataAll);
    } else if (checkedFilter == 3) {
      _lstData = CompletedTenantResponse.getLast15To30DaysList(_lstDataAll);
    } else if (checkedFilter == 4) {
      _lstData = CompletedTenantResponse.getBefore30DaysList(_lstDataAll);
    } else {
      _lstData = _lstDataAll;
