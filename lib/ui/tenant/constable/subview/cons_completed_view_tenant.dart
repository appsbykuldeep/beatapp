...
import 'package:beatapp/model/response/completed_tenant_cons_response.dart';
...
class ConsCompletedFragmentTenant extends StatefulWidget {
  const ConsCompletedFragmentTenant({Key? key}) : super(key: key);

  @override
  State<ConsCompletedFragmentTenant> createState() =>
      _ConsCompletedFragmentTenantState();
}

class _ConsCompletedFragmentTenantState
    extends BaseFullState<ConsCompletedFragmentTenant> {
  List<CompletedTenantConsResponse> _lstData = [];
  final List<CompletedTenantConsResponse> _lstDataAll = [];
  ...
  void _getTenantVerificationList() async {
    ...
    var response = await HttpRequst.postRequestWithTokenAndBody(
        context, EndPoints.COMPLETED_TENANT_LIST_TO_CONS, data, true);
    if (response.statusCode == 200) {
      try {
        for (Map<String, dynamic> i in response.data) {
          var data = CompletedTenantConsResponse.fromJson(i);
          _lstData.add(data);
  ...
  void savePdf({required List<CompletedTenantConsResponse> finalData}) async {
        ...
        onTap: () {
          if (_lstData.isNotEmpty) {
            showDownloadOption(onXlsxClick: () {
              CompletedTenantConsResponse.generateExcel(context, _lstData);
            }, onPdfClick: () {
      ...
      child: Container(
          ...
          child: Row(
            children: [
              ...
              SizedBox(
                width: 130,
                child: Text((data.cOMPDT ?? "").split(" ")[0], maxLines: 2),
              ),
              SizedBox(
                width: 130,
                    ...
                    Container(
                      ...
                      child: InkWell(
                        onTap: () {
                          ...
                          _lstData =
                              CompletedTenantConsResponse.getDataFromToDateList(
  ...
  setFilterData() {
    ...
    if (checkedFilter == 1) {
      ...
      _lstData = CompletedTenantConsResponse.getLast15DaysList(_lstDataAll);
    } else if (checkedFilter == 3) {
      _lstData = CompletedTenantConsResponse.getLast15To30DaysList(_lstDataAll);
    } else if (checkedFilter == 4) {
      _lstData = CompletedTenantConsResponse.getBefore30DaysList(_lstDataAll);
    } else {
      _lstData = _lstDataAll;
