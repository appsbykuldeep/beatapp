...
class CompletedFragmentDomestic extends StatefulWidget {
  const CompletedFragmentDomestic({Key? key}) : super(key: key);

  @override
  State<CompletedFragmentDomestic> createState() =>
      _CompletedFragmentDomesticState();
}

class _CompletedFragmentDomesticState extends State<CompletedFragmentDomestic> {
  List<CompletedDomesticResponse> _lstData = [];
  List<CompletedDomesticResponse> _lstDataAll = [];
  ...
  void _getDomesticVerificationList() async {
    ...
    var response = await HttpRequst.postRequestWithTokenAndBody(
        context, EndPoints.END_POINT_COMPLETED_DOMESTIC_LIST, data, true);
    _lstData = [];
    _lstDataAll = [];
        ...
        bool res = await Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) => DomesticVerificationDetailActivity(
                  data: {"DOMESTIC_SR_NUM": data.dOMESTICSRNUM, "action": "1"}),
            ));
        if (res) {
          _getDomesticVerificationList();
        }
