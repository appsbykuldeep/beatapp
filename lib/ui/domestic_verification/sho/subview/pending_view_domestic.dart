...
import 'package:beatapp/model/response/pending_domestic_response.dart';
...
class PendingFragmentDomestic extends StatefulWidget {
  const PendingFragmentDomestic({Key? key}) : super(key: key);

  @override
  State<PendingFragmentDomestic> createState() =>
      _PendingFragmentDomesticState();
}

class _PendingFragmentDomesticState extends State<PendingFragmentDomestic> {
  List<PendingDomesticResponse> _lstData = [];
  final List<PendingDomesticResponse> _lstDataAll = [];
  ...
  void _getDomesticVerificationList() async {
    ...
    var response = await HttpRequst.postRequestWithTokenAndBody(
        context, EndPoints.END_POINT_ASSIGNED_DOMESTIC_LIST, data, true);
    if (response.statusCode == 200) {
      try {
        for (Map<String, dynamic> i in response.data) {
          var data = PendingDomesticResponse.fromJson(i);
          _lstData.add(data);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: InkWell(
        onTap: () {
          PendingDomesticResponse.generateExcel(context, _lstData);
        },
        child: Container(
          height: 50,
          color: Colors.white,
                    ...
                    Container(
                      ...
                      child: InkWell(
                        onTap: () {
                          ...
                          _lstData =
                              PendingDomesticResponse.getDataFromToDateList(
  ...
  setFilterData() {
    ...
    if (checkedFilter == 1) {
      ...
      _lstData = PendingDomesticResponse.getLast15DaysList(_lstDataAll);
    } else if (checkedFilter == 3) {
      _lstData = PendingDomesticResponse.getLast15To30DaysList(_lstDataAll);
    } else if (checkedFilter == 4) {
      _lstData = PendingDomesticResponse.getBefore30DaysList(_lstDataAll);
    } else {
      _lstData = _lstDataAll;
