...
import 'package:beatapp/model/response/unassigned_domestic_response.dart';
import 'package:beatapp/ui/dialog/dialog_helper.dart';
import 'package:beatapp/ui/domestic_verification/sho/domestic_verification_assign_view.dart';
import 'package:beatapp/utility/message_utility.dart';
import 'package:beatapp/utility/resource_provider.dart';
import 'package:flutter/material.dart';

class UnassignedFragmentDomestic extends StatefulWidget {
  const UnassignedFragmentDomestic({Key? key}) : super(key: key);

  @override
  State<UnassignedFragmentDomestic> createState() =>
      _UnassignedFragmentDomesticState();
}

class _UnassignedFragmentDomesticState
    extends State<UnassignedFragmentDomestic> {
  List<UnassignedDomesticResponse> _lstData = [];
  final List<UnassignedDomesticResponse> _lstDataAll = [];
  ...
  void _getDomesticVerificationList() async {
    ...
    var response = await HttpRequst.postRequestWithTokenAndBody(
        context, EndPoints.END_POINT_UNASSIGNED_DOMESTIC_LIST, data, true);
    if (response.statusCode == 200) {
      _lstData = [];
      try {
        for (Map<String, dynamic> i in response.data) {
          var data = UnassignedDomesticResponse.fromJson(i);
          _lstData.add(data);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: InkWell(
        onTap: () {
          UnassignedDomesticResponse.generateExcel(context, _lstData);
        },
        child: Container(
          height: 50,
          color: Colors.white,
    ...
    return InkWell(
      onTap: () async {
        bool result = await Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => DomesticVerificationAssignActivity(
                    ...
                    Container(
                      ...
                      child: InkWell(
                        onTap: () {
                          ...
                          _lstData =
                              UnassignedDomesticResponse.getDataFromToDateList(
  ...
  setFilterData() {
    ...
    if (checkedFilter == 1) {
      ...
      _lstData = UnassignedDomesticResponse.getLast15DaysList(_lstDataAll);
    } else if (checkedFilter == 3) {
      _lstData = UnassignedDomesticResponse.getLast15To30DaysList(_lstDataAll);
    } else if (checkedFilter == 4) {
      _lstData = UnassignedDomesticResponse.getBefore30DaysList(_lstDataAll);
    } else {
      _lstData = _lstDataAll;
