...
import 'package:beatapp/model/response/pending_domestic_cons_response.dart';
import 'package:beatapp/ui/dialog/dialog_helper.dart';
import 'package:beatapp/ui/domestic_verification/constable/cons_domestic_info_submit_view.dart';
...
class ConsPendingFragmentDomestic extends StatefulWidget {
  const ConsPendingFragmentDomestic({Key? key}) : super(key: key);

  @override
  State<ConsPendingFragmentDomestic> createState() =>
      _ConsPendingFragmentDomesticState();
}

class _ConsPendingFragmentDomesticState
    extends BaseFullState<ConsPendingFragmentDomestic> {
  List<PendingDomesticConsResponse> _lstData = [];
  List<PendingDomesticConsResponse> _lstDataAll = [];
  ...
  void _getDomesticVerificationList() async {
    ...
    var response = await HttpRequst.postRequestWithTokenAndBody(
        context, EndPoints.END_POINT_ASSIGNED_DOMESTIC_CONS, data, true);
    if (response.statusCode == 200) {
      _lstData = [];
      _lstDataAll = [];
      try {
        for (Map<String, dynamic> i in response.data) {
          var data = PendingDomesticConsResponse.fromJson(i);
          _lstData.add(data);
  ...
  void savePdf({required List<PendingDomesticConsResponse> finalData}) async {
        ...
        onTap: () {
          if (_lstData.isNotEmpty) {
            showDownloadOption(onXlsxClick: () {
              PendingDomesticConsResponse.generateExcel(context, _lstData);
            }, onPdfClick: () {
    ...
    return InkWell(
      onTap: () async {
        bool res = await Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => ConsDomesticInfoSubmitActivity(
                    ...
                    Container(
                      ...
                      child: InkWell(
                        onTap: () {
                          ...
                          _lstData =
                              PendingDomesticConsResponse.getDataFromToDateList(
  ...
  setFilterData() {
    ...
    if (checkedFilter == 1) {
      ...
      _lstData = PendingDomesticConsResponse.getLast15DaysList(_lstDataAll);
    } else if (checkedFilter == 3) {
      _lstData = PendingDomesticConsResponse.getLast15To30DaysList(_lstDataAll);
    } else if (checkedFilter == 4) {
      _lstData = PendingDomesticConsResponse.getBefore30DaysList(_lstDataAll);
    } else {
      _lstData = _lstDataAll;
