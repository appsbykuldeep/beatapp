...
class ShoCompletedCharacterListFragment extends StatefulWidget {
  const ShoCompletedCharacterListFragment({Key? key}) : super(key: key);

  @override
  State<ShoCompletedCharacterListFragment> createState() =>
      _ShoCompletedCharacterListFragmentState();
}

class _ShoCompletedCharacterListFragmentState
    extends State<ShoCompletedCharacterListFragment> {
  ...
  void _getCharacterCertList() async {
    ...
    var response = await HttpRequst.postRequestWithTokenAndBody(
        context, EndPoints.COMPLETED_CHARACTER_LIST_SHO, data, true);
        ...
        Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) => CharacterCertificateDetailActivity(
                data: {"CHARACTER_SR_NUM": data.srNum, "id": 3},
              ),
            ));
      },
      child: Container(
          ...
          child: Row(
            children: [
              ...
              SizedBox(
                width: 130,
                child: Text(data.completeDt ?? "", maxLines: 2),
              ),
              SizedBox(
                width: 130,
  ...
  setFilterData() {
    ...
    if (checkedFilter == 1) {
      ...
      lstData = CharacterCertificate.getLast15DaysListComplete(lstDataAll);
    } else if (checkedFilter == 3) {
      lstData = CharacterCertificate.getLast15To30DaysListComplete(lstDataAll);
