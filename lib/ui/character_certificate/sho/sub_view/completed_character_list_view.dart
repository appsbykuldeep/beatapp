...
class CompletedCharacterListFragment extends StatefulWidget {
  const CompletedCharacterListFragment({Key? key}) : super(key: key);

  @override
  State<CompletedCharacterListFragment> createState() =>
      _CompletedCharacterListFragmentState();
}

class _CompletedCharacterListFragmentState
    extends State<CompletedCharacterListFragment> {
  ...
  void _getCharacterCertList() async {
    ...
    var response = await HttpRequst.postRequestWithTokenAndBody(context,
        EndPoints.END_POINT_GET_COMPLETED_CHARACTER_LIST_PS, data, true);
    lstData = [];
    lstDataAll = [];
        ...
        bool res = await Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) => CharacterCertificateDetailActivity(
                data: {"CHARACTER_SR_NUM": data.srNum, "id": 2, "action": "1"},
              ),
            ));
        if (res) {
          _getCharacterCertList();
