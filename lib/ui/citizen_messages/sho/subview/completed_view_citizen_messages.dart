...
class CompletedFragmentCitizenMessages extends StatefulWidget {
  const CompletedFragmentCitizenMessages({Key? key}) : super(key: key);

  @override
  State<CompletedFragmentCitizenMessages> createState() =>
      _CompletedFragmentCitizenMessagesState();
}

class _CompletedFragmentCitizenMessagesState
    extends State<CompletedFragmentCitizenMessages> {
  List<PendingCitizenMessagesResponse> _lstMsg = [];
  List<PendingCitizenMessagesResponse> _lstMsgAll = [];
  ...
  void _getMessageList() async {
    ...
    var response = await HttpRequst.postRequestWithTokenAndBody(
        context, EndPoints.COMPLETED_CS_SHARED_INFO_LIST, data, true);
    _lstMsg = [];
    _lstMsgAll = [];
          ...
          InkWell(
            onTap: () {
              PendingCitizenMessagesResponse.generateExcel(
                  context, _lstMsg, "CompletedCitizenMsg");
            },
            child: Container(
        ...
        bool res = await Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) => CitizenMessagesDetailActivity(
                data: {"PERSONID": data.pERSONID, "action": "1"},
              ),
            ));
        if (res) {
          _getMessageList();
