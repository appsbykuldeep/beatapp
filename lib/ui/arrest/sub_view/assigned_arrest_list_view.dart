...
import 'package:beatapp/ui/arrest/arrest_accuse_view.dart';
...
class AssignedArrestListFragment extends StatefulWidget {
  final data;

  const AssignedArrestListFragment({Key? key, required this.data})
      : super(key: key);

  @override
  State<AssignedArrestListFragment> createState() =>
      _AssignedArrestListFragmentState(data);
}

class _AssignedArrestListFragmentState
    extends BaseFullState<AssignedArrestListFragment> {
  List<Arrest> _lstArrest = [];
  List<Arrest> _lstArrestAll = [];
  ...
  void _getArrestList() async {
    ...
    if (role == "23") {
      response = await HttpRequst.postRequestWithTokenAndBody(context,
          EndPoints.END_POINT_GET_ASSIGNED_ARREST_LIST_BEAT, data, true);
    } else {
      response = await HttpRequst.postRequestWithTokenAndBody(context,
          EndPoints.END_POINT_GET_ASSIGNED_ARREST_LIST_SHO, data, true);
    }
  ...
  void savePdf({required List<Arrest> finalData}) async {
    ...
    await CameraAndFileProvider.saveFile(
        "AssignedCriminals", await pdf.save(), ".pdf");
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(getTranlateString("assigned_arrest")),
        ...
        onTap: () {
          if (_lstArrest.isNotEmpty) {
            showDownloadOption(onPdfClick: () {
              ...
              Arrest.generateExcel(context, _lstArrest, "assigned_criminal");
              //savePdf(finalData: _lstArrest);
            });
          }
        },
  ...
  Widget getRowForConst(int index) {
    ...
    return InkWell(
      onTap: () async {
        StatefulWidget page;
        if (role == "23") {
          page = ArrestAccuseActivity(arrest: data);
        } else {
          page = ArrestDetailActivity(arrest: data);
        }
