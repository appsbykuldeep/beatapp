...
import 'package:beatapp/model/response/unassigned_employee_response.dart';
import 'package:beatapp/ui/dialog/dialog_helper.dart';
import 'package:beatapp/ui/employee_verification/sho/employee_verification_assign_view.dart';
import 'package:beatapp/utility/message_utility.dart';
import 'package:beatapp/utility/resource_provider.dart';
import 'package:flutter/material.dart';
import 'package:beatapp/api/api_connection.dart' as HttpRequst;

class UnassignedFragmentEmployee extends StatefulWidget {
  const UnassignedFragmentEmployee({Key? key}) : super(key: key);

  @override
  State<UnassignedFragmentEmployee> createState() =>
      _UnassignedFragmentEmployeeState();
}

class _UnassignedFragmentEmployeeState
    extends State<UnassignedFragmentEmployee> {
  List<UnassignedEmployeeResponse> _lstData = [];
  final List<UnassignedEmployeeResponse> _lstDataAll = [];
  ...
  void _getEmployeeVerificationList() async {
    ...
    var response = await HttpRequst.postRequestWithTokenAndBody(
        context, EndPoints.END_POINT_UNASSIGNED_EMPLOYEE_LIST, data, true);
    if (response.statusCode == 200) {
      try {
        for (Map<String, dynamic> i in response.data) {
          var data = UnassignedEmployeeResponse.fromJson(i);
          _lstData.add(data);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
                                ...
                                child: Row(
                                  children: [
                                    ...
                                    SizedBox(
                                      width: 130,
                                      child: Text(
                                        getTranlateString("assigned_status"),
                                        style: const TextStyle(
                                            fontWeight: FontWeight.bold,
                                            fontSize: 12),
                                      ),
      ...
      bottomNavigationBar: InkWell(
        onTap: () {
          UnassignedEmployeeResponse.generateExcel(context, _lstData);
        },
        child: Container(
          height: 50,
          color: Colors.white,
    ...
    return InkWell(
      onTap: () async {
        await Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => EmployeeVerificationAssignActivity(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            SizedBox(
              width: 130,
              child: Text(data.rEGISTRATIONDATE ?? "", maxLines: 2),
            ),
            SizedBox(
              width: 130,
                    ...
                    Container(
                      ...
                      child: InkWell(
                        onTap: () {
                          ...
                          _lstData =
                              UnassignedEmployeeResponse.getDataFromToDateList(
  ...
  setFilterData() {
    ...
    if (checkedFilter == 1) {
      ...
      _lstData = UnassignedEmployeeResponse.getLast15DaysList(_lstDataAll);
    } else if (checkedFilter == 3) {
      _lstData = UnassignedEmployeeResponse.getLast15To30DaysList(_lstDataAll);
    } else if (checkedFilter == 4) {
      _lstData = UnassignedEmployeeResponse.getBefore30DaysList(_lstDataAll);
    } else {
      _lstData = _lstDataAll;
