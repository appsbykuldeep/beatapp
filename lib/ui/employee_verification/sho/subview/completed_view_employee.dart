...
import 'package:beatapp/model/response/completed_employee_response.dart';
...
class CompletedFragmentEmployee extends StatefulWidget {
  const CompletedFragmentEmployee({Key? key}) : super(key: key);

  @override
  State<CompletedFragmentEmployee> createState() =>
      _CompletedFragmentEmployeeState();
}

class _CompletedFragmentEmployeeState extends State<CompletedFragmentEmployee> {
  List<CompletedEmployeeResponse> _lstData = [];
  List<CompletedEmployeeResponse> _lstDataAll = [];
  ...
  void _getEmployeeVerificationList() async {
    ...
    var response = await HttpRequst.postRequestWithTokenAndBody(
        context, EndPoints.END_POINT_COMPLETED_EMPLOYEE_LIST, data, true);
    _lstData = [];
    _lstDataAll = [];
    if (response.statusCode == 200) {
      try {
        for (Map<String, dynamic> i in response.data) {
          var data = CompletedEmployeeResponse.fromJson(i);
          _lstData.add(data);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: InkWell(
        onTap: () {
          CompletedEmployeeResponse.generateExcel(context, _lstData);
        },
        child: Container(
          height: 50,
          color: Colors.white,
        ...
        Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) => EmployeeVerificationDetailActivity(
                  data: {"EMPLOYEE_SR_NUM": data.eMPLOYEESRNUM, "action": "1"}),
            ));
      },
      child: Container(
                    ...
                    Container(
                      ...
                      child: InkWell(
                        onTap: () {
                          ...
                          _lstData =
                              CompletedEmployeeResponse.getDataFromToDateList(
  ...
  setFilterData() {
    ...
    if (checkedFilter == 1) {
      ...
      _lstData = CompletedEmployeeResponse.getLast15DaysList(_lstDataAll);
    } else if (checkedFilter == 3) {
      _lstData = CompletedEmployeeResponse.getLast15To30DaysList(_lstDataAll);
    } else if (checkedFilter == 4) {
      _lstData = CompletedEmployeeResponse.getBefore30DaysList(_lstDataAll);
    } else {
      _lstData = _lstDataAll;
