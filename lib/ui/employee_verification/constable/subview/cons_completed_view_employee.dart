...
import 'package:beatapp/model/response/completed_employee_cons_response.dart';
...
class ConsCompletedFragmentEmployee extends StatefulWidget {
  const ConsCompletedFragmentEmployee({Key? key}) : super(key: key);

  @override
  State<ConsCompletedFragmentEmployee> createState() =>
      _ConsCompletedFragmentEmployeeState();
}

class _ConsCompletedFragmentEmployeeState
    extends BaseFullState<ConsCompletedFragmentEmployee> {
  List<CompletedEmployeeConsResponse> _lstData = [];
  List<CompletedEmployeeConsResponse> _lstDataAll = [];
  ...
  void _getEmployeeVerificationList() async {
    ...
    var response = await HttpRequst.postRequestWithTokenAndBody(
        context, EndPoints.END_POINT_COMPLETED_EMPLOYEE_CONS, data, true);
    if (response.statusCode == 200) {
      _lstData = [];
      _lstDataAll = [];
      try {
        for (Map<String, dynamic> i in response.data) {
          var data = CompletedEmployeeConsResponse.fromJson(i);
          _lstData.add(data);
  ...
  void savePdf({required List<CompletedEmployeeConsResponse> finalData}) async {
        ...
        onTap: () {
          if (_lstData.isNotEmpty) {
            showDownloadOption(onPdfClick: () {
              ...
              CompletedEmployeeConsResponse.generateExcel(context, _lstData);
            });
          }
        },
        child: Container(
                    ...
                    Container(
                      ...
                      child: InkWell(
                        onTap: () {
                          ...
                          _lstData = CompletedEmployeeConsResponse
  ...
  setFilterData() {
    ...
    if (checkedFilter == 1) {
      ...
      _lstData = CompletedEmployeeConsResponse.getLast15DaysList(_lstDataAll);
    } else if (checkedFilter == 3) {
      _lstData =
          CompletedEmployeeConsResponse.getLast15To30DaysList(_lstDataAll);
    } else if (checkedFilter == 4) {
      _lstData = CompletedEmployeeConsResponse.getBefore30DaysList(_lstDataAll);
    } else {
      _lstData = _lstDataAll;
