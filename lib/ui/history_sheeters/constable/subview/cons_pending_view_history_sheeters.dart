...
import 'package:beatapp/ui/history_sheeters/constable/history_sheeter_detail_submit_view.dart';
...
class ConsPendingFragmentHistorySheeters extends StatefulWidget {
  const ConsPendingFragmentHistorySheeters({Key? key}) : super(key: key);

  @override
  State<ConsPendingFragmentHistorySheeters> createState() =>
      _ConsPendingFragmentHistorySheetersState();
}

class _ConsPendingFragmentHistorySheetersState
    extends BaseFullState<ConsPendingFragmentHistorySheeters> {
  List<HistorySheetersDetailsResponse> _lstData = [];
  List<HistorySheetersDetailsResponse> _lstDataAll = [];
  ...
  void _getHSList() async {
    ...
    var response = await HttpRequst.postRequestWithTokenAndBody(
        context, EndPoints.GET_HISTORY_SHEETERS_DETAILS, data, true);
  ...
  getSelectedList() {
    if (isActiveSelcted) {
      _lstData = HistorySheetersDetailsResponse.getActiveList(_lstDataAll);
    } else {
      _lstData = HistorySheetersDetailsResponse.getInActiveList(_lstDataAll);
    }
    setState(() {});
  }
  ...
  void savePdf(
      {required List<HistorySheetersDetailsResponse> finalData}) async {
    ...
    return InkWell(
      onTap: () async {
        bool res = await Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => HistorySheeterDetailSubmitActivity(
